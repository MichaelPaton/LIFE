...
var NRS = (function(NRS, $, undefined) {
	...
	NRS.forms.sellAlias = function($modal) {
		...
		if (data.modal == "cancel_alias_sale") {
			...
			if (data.recipient == NRS.genesisRS) {
				if (!data.priceLIFE || data.priceLIFE == "0") {
					return {
						"error": $.t("error_not_specified", {
		...
		NRS.sendRequest("getAlias", {
			...
			if (response.errorCode) {
				...
				if (!("priceNQT" in response)) {
					...
					$modal.find("input[name=amountLIFE]").val(NRS.convertToLIFE(response.priceNQT)).prop("readonly", true);
				}
			}
		}, false);
	});
	...
	NRS.forms.buyAliasError = function() {
		$("#buy_alias_modal").find("input[name=priceLIFE]").prop("readonly", false);
	}
		...
		if (alias) {
			...
			NRS.sendRequest("getAlias", {
				...
				if (response.errorCode) {
					...
					} else if ((aliasURI = /acct:(.*)@life/.exec(response.aliasURI)) || (aliasURI = /nacc:(.*)/.exec(response.aliasURI))) {
	...
	NRS.forms.setAlias = function($modal) {
		...
		if (data.type == "account") {
			if (!(/acct:(.*)@life/.test(data.aliasURI)) && !(/nacc:(.*)/.test(data.aliasURI))) {
				if (/^(LIFE\-)/i.test(data.aliasURI)) {
					...
					if (!address.set(data.aliasURI)) {
						...
						data.aliasURI = "acct:" + data.aliasURI + "@life";
					}
	...
	function setAliasType(type, uri) {
		...
		if (type == "uri") {
			...
			$("#register_alias_uri").val("").mask("LIFE-****-****-****-*****");

			if (uri) {
				var match = uri.match(/acct:(.*)@life/i);
				if (!match) {
				...
				} else if (!/^LIFE\-[A-Z0-9]{4}\-[A-Z0-9]{4}\-[A-Z0-9]{4}\-[A-Z0-9]{5}/i.test(uri)) {
					uri = NRS.accountRS;
				}
					...
					if ("priceNQT" in response) {
						if (response.buyer == NRS.account) {
							message = $.t("alias_sale_direct_offer", {
								"life": NRS.formatAmount(response.priceNQT)
