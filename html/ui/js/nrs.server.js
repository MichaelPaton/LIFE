...
var NRS = (function(NRS, $, undefined) {
	...
	NRS.sendRequest = function(requestType, data, callback, async) {
		...
		//convert LIFE to NQT...
		try {
			var lifeFields = ["feeLIFE", "amountLIFE", "priceLIFE", "refundLIFE", "discountLIFE"];

			for (var i = 0; i < lifeFields.length; i++) {
				var lifeField = lifeFields[i];
				var field = lifeField.replace("LIFE", "");

				if (lifeField in data) {
					data[field + "NQT"] = NRS.convertToNQT(data[lifeField]);
					delete data[lifeField];
				}
			}
		} catch (err) {
			if (callback) {
		...
		if (requestType == "getAccountId") {
			...
			var lifeAddress = new LifeAddress();

			if (lifeAddress.set(accountId)) {
				var accountRS = lifeAddress.toString();
			} else {
				var accountRS = "";
			}
	...
	NRS.processAjaxRequest = function(requestType, data, callback, async) {
		...
		var url = NRS.server + "/life?requestType=" + requestType;
	...
	NRS.verifyAndSignTransactionBytes = function(transactionBytes, signature, requestType, data) {
		...
		if (!("recipient" in data)) {
			...
			data.recipientRS = "LIFE-MRCC-2YLS-8M54-3CMAJ";
		}
	...
	NRS.broadcastTransactionBytes = function(transactionData, callback, originalResponse, originalData) {
		$.ajax({
			url: NRS.server + "/life?requestType=broadcastTransaction",
			crossDomain: true,
			dataType: "json",
			type: "POST",
			timeout: 30000,
