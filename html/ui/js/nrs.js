...
var NRS = (function(NRS, $, undefined) {
	...
	NRS.state = {};
	NRS.blocks = [];
	NRS.genesis = "2314846985700083696";
	NRS.genesisRS = "LIFE-6YYF-SPXX-7XVB-5PSB8";
	...
	NRS.init = function() {
		if (window.location.port && window.location.port != "16876") {
			$(".testnet_only").hide();
		} else {
			NRS.isTestNet = true;
		...
		$("#dgs_search_account_top, #dgs_search_account_center").mask("LIFE-****-****-****-*****", {
			"unmask": false
		});
	...
	NRS.updateAccountLeasingStatus = function() {
		...
		if (NRS.accountInfo.effectiveBalanceLIFE == 0) {
