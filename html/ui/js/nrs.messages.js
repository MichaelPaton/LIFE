...
var NRS = (function(NRS, $, undefined) {
			...
			for (var i = 0; i < messages.length; i++) {
				...
				if (decoded !== false) {
					...
					if (extra == "to_decrypt") {
						...
						if (type == "payment") {
							decoded = "<strong>+" + NRS.formatAmount(messages[i].amountNQT) + " LIFE</strong><br />" + decoded;
						}
	...
	NRS.forms.sendMessageComplete = function(response, data) {
		...
		if (NRS.currentPage == "messages") {
			var date = new Date(Date.UTC(2014, 12, 7, 0, 0, 0, 0)).getTime();
