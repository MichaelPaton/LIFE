...
var NRS = (function(NRS, $, undefined) {
	...
	NRS.calculateOrderPricePerWholeQNT = function(price, decimals, returnAsObject) {
		...
		return NRS.convertToLIFE(price.multiply(new BigInteger("" + Math.pow(10, decimals))), returnAsObject);
	}
	...
	NRS.calculateOrderTotal = function(quantityQNT, priceNQT) {
		...
		return NRS.convertToLIFE(quantityQNT.multiply(priceNQT));
	}
	...
	NRS.convertToLIFE = function(amount, returnAsObject) {
		var negative = "";
		var afterComma = "";
	...
	NRS.formatAmount = function(amount, round, no_escaping) {
		...
		if (typeof amount == "object") {
			var params = NRS.convertToLIFE(amount, true);
	...
	NRS.formatTimestamp = function(timestamp, date_only) {
		if (typeof timestamp == "object") {
			...
			var date = new Date(Date.UTC(2014, 12, 7, 0, 0, 0, 0) + timestamp * 1000);
		}
		...
		*/
		...
		for (var key in data) {
			...
			if (/_formatted_html$/i.test(key)) {
				...
				value = NRS.formatAmount(new BigInteger(String(value))) + " LIFE";
