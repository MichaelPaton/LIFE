...
var NRS = (function(NRS, $, undefined) {
	...
	NRS.forms.addAssetBookmark = function($modal) {
		...
		if (!/^\d+$/.test(data.id) && !/^LIFE\-/i.test(data.id)) {
			return {
				"error": $.t("error_asset_or_account_id_invalid")
			};
		}

		if (/^LIFE\-/i.test(data.id)) {
	...
	NRS.loadAsset = function(asset, refresh) {
		...
		if (!refresh) {
			...
			$("#sell_asset_for_life").html($.t("sell_asset_for_life", {
				"assetName": String(asset.name).escapeHTML()
			}));
			$("#buy_asset_with_life").html($.t("buy_asset_with_life", {
				"assetName": String(asset.name).escapeHTML()
			}));
		...
		if (NRS.accountInfo.unconfirmedBalanceNQT == "0") {
			$("#your_life_balance").html("0");
			$("#buy_automatic_price").addClass("zero").removeClass("nonzero");
		} else {
			$("#your_life_balance").html(NRS.formatAmount(NRS.accountInfo.unconfirmedBalanceNQT));
	...
	$("#asset_exchange_bid_orders_table tbody, #asset_exchange_ask_orders_table tbody").on("click", "td", function(e) {
		...
		try {
			...
			$("#" + type + "_asset_total").val(NRS.convertToLIFE(totalNQT));
		} catch (err) {
			return;
		}
	...
	$("#sell_automatic_price, #buy_automatic_price").on("click", function(e) {
		try {
			...
			if (price.cmp(new Big("0")) <= 0) {
				...
				$("#" + type + "_asset_price").val(NRS.convertToLIFE(price.toString()));
			}
			...
			$("#" + type + "_asset_total").val(NRS.convertToLIFE(total.toString()));
	...
	$("#asset_order_modal").on("show.bs.modal", function(e) {
		...
		try {
			...
			var totalLIFE = NRS.formatAmount(NRS.calculateOrderTotalNQT(quantityQNT, priceNQT, NRS.currentAsset.decimals), false, true);
		} catch (err) {
		...
		if (orderType == "buy") {
			var description = $.t("buy_order_description", {
				...
				"life": NRS.formatAmount(priceNQTPerWholeQNT)
			});
			var tooltipTitle = $.t("buy_order_description_help", {
				"life": NRS.formatAmount(priceNQTPerWholeQNT, false, true),
		...
		$("#asset_order_total").html(totalLIFE + " LIFE");
		$("#asset_order_fee_paid").html(NRS.formatAmount(feeNQT) + " LIFE");
