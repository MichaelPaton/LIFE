		...
		if (currentTime - NRS.newsRefresh > 60 * 60 * 10) { //10 minutes before refreshing..
			...
			var settingsReddit = {
				...
				"filter": function(entry, tokens) {
					return tokens.title.indexOf("Donations toward") == -1 && tokens.title.indexOf("LIFE tipping bot has arrived") == -1
				}
			};

			$("#lifeforum_news").rss("https://lifeforum.org/index.php?type=rss;action=.xml;sa=news;", settings, NRS.newsLoaded);
			$("#reddit_news").rss("http://www.reddit.com/r/LIFE/.rss", settingsReddit, NRS.newsLoaded);
			$("#lifecoin_blogspot_news").rss("http://lifecoin.blogspot.com/feeds/posts/default", settings, NRS.newsLoaded);
			$("#lifeer_news").rss("http://lifeer.org/feed/", settings, NRS.newsLoaded);
			$("#lifecommunity_news").rss("http://www.lifecommunity.org/rss.xml", settings, NRS.newsLoaded);
		}
