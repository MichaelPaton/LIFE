...
public abstract class EntityDbTable<T> extends DerivedDbTable {
    ...
    public final void checkAvailable(int height) {
        if (multiversion && height < LifeCoin.getBlockchainProcessor().getMinRollbackHeight()) {
            throw new IllegalArgumentException("Historical data as of height " + height +" not available, set life.trimDerivedTables=false and re-scan");
        }
    }
