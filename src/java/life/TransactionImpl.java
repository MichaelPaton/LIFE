...
final class TransactionImpl implements Transaction {
    ...
    static final class BuilderImpl implements Builder {
        ...
        @Override
        public TransactionImpl build() throws LifeException.NotValidException {
    ...
    private TransactionImpl(BuilderImpl builder) throws LifeException.NotValidException {
        ...
        int effectiveHeight = (height < Integer.MAX_VALUE ? height : LifeCoin.getBlockchain().getHeight());
        long minimumFeeNQT = type.minimumFeeNQT(effectiveHeight, appendagesSize);
        if (builder.feeNQT > 0 && builder.feeNQT < minimumFeeNQT) {
            throw new LifeException.NotValidException(String.format("Requested fee %d less than the minimum fee %d",
        ...
        if ((timestamp == 0 && Arrays.equals(senderPublicKey, Genesis.CREATOR_PUBLIC_KEY))
                ...
                || type == null) {
            throw new LifeException.NotValidException("Invalid transaction parameters:\n type: " + type + ", timestamp: " + timestamp
                    + ", deadline: " + deadline + ", fee: " + feeNQT + ", amount: " + amountNQT);
        }

        if (attachment == null || type != attachment.getTransactionType()) {
            throw new LifeException.NotValidException("Invalid attachment " + attachment + " for transaction of type " + type);
        }

        if (! type.canHaveRecipient()) {
            if (recipientId != 0 || getAmountNQT() != 0) {
                throw new LifeException.NotValidException("Transactions of this type must have recipient == 0, amount == 0");
            }
        }

        if (type.mustHaveRecipient() && version > 0) {
            if (recipientId == 0) {
                throw new LifeException.NotValidException("Transactions of this type must have a valid recipient");
            }
        }

        for (Appendix.AbstractAppendix appendage : appendages) {
            if (! appendage.verifyVersion(this.version)) {
                throw new LifeException.NotValidException("Invalid attachment version " + appendage.getVersion()
    ...
    public Block getBlock() {
        if (block == null && blockId != 0) {
            block = LifeCoin.getBlockchain().getBlock(blockId);
        }
        return block;
    }
    ...
    public byte[] getBytes() {
        try {
            ...
            if (useNQT()) {
                ...
                buffer.putInt((int) (amountNQT / Constants.ONE_LIFE));
                buffer.putInt((int) (feeNQT / Constants.ONE_LIFE));
                if (referencedTransactionFullHash != null) {
    ...
    static TransactionImpl parseTransaction(byte[] bytes) throws LifeException.ValidationException {
        try {
        ...
        } catch (LifeException.NotValidException|RuntimeException e) {
    ...
    */
    ...
    static TransactionImpl parseTransaction(JSONObject transactionData) throws LifeException.NotValidException {
        try {
            ...
            if (transactionType == null) {
                throw new LifeException.NotValidException("Invalid transaction type: " + type + ", " + subtype);
            }
    ...
    public void sign(String secretPhrase) {
        if (signature != null) {  //TODO comment first run
    ...
    private boolean useNQT() {
        return this.height > Constants.NQT_BLOCK
                ...
                || LifeCoin.getBlockchain().getHeight() >= Constants.NQT_BLOCK);
    }
    ...
    @Override
    public void validate() throws LifeException.ValidationException {
        for (Appendix.AbstractAppendix appendage : appendages) {
            appendage.validate(this);
        }
        long minimumFeeNQT = type.minimumFeeNQT(LifeCoin.getBlockchain().getHeight(), appendagesSize);
        if (feeNQT < minimumFeeNQT) {
            throw new LifeException.NotCurrentlyValidException(String.format("Transaction fee %d less than minimum fee %d at height %d",
                    feeNQT, minimumFeeNQT, LifeCoin.getBlockchain().getHeight()));
        }
        if (LifeCoin.getBlockchain().getHeight() >= Constants.PUBLIC_KEY_ANNOUNCEMENT_BLOCK) {
            ...
            if (recipientId != 0 /* && recipientId != getSenderId() */) {
                ...
                if ((recipientAccount == null || recipientAccount.getPublicKey() == null) && publicKeyAnnouncement == null) {
                    throw new LifeException.NotCurrentlyValidException("Recipient account does not have a public key, must attach a public key announcement");
                }
            }
        }
    }
