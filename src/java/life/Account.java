...
import life.db.DbUtils;
import life.db.DerivedDbTable;
import life.db.VersionedEntityDbTable;
...
public final class Account {
    ...
    private Account(long id) {
        ...
        this.creationHeight = LifeCoin.getBlockchain().getHeight();
    ...
    public long getEffectiveBalanceLIFE() {
        ...
        if (lastBlock.getHeight() < Constants.TRANSPARENT_FORGING_BLOCK_3
            ...
            if (this.creationHeight == 0) {
                return getBalanceNQT() / Constants.ONE_LIFE;
            }
            ...
            return (getBalanceNQT() - receivedInlastBlock) / Constants.ONE_LIFE;
        }
        if (lastBlock.getHeight() < currentLeasingHeightFrom) {
            return (getGuaranteedBalanceNQT(1440) + getLessorsGuaranteedBalanceNQT()) / Constants.ONE_LIFE;
        }
        return getLessorsGuaranteedBalanceNQT() / Constants.ONE_LIFE;
    }
    ...
    public DbIterator<Account> getLessors() {
        return accountTable.getManyBy(getLessorsClause(LifeCoin.getBlockchain().getHeight()), 0, -1);
    }
    ...
    public long getGuaranteedBalanceNQT(final int numberOfConfirmations) {
        return getGuaranteedBalanceNQT(numberOfConfirmations, LifeCoin.getBlockchain().getHeight());
    }
    ...
    void leaseEffectiveBalance(long lesseeId, short period) {
        ...
        if (lessee != null && lessee.getPublicKey() != null) {
            int height = LifeCoin.getBlockchain().getHeight();
