...
final class BlockImpl implements Block {
    ...
    BlockImpl(int version, int timestamp, long previousBlockId, long totalAmountNQT, long totalFeeNQT, int payloadLength, byte[] payloadHash,
            ...
            throws LifeException.ValidationException {

        if (payloadLength > Constants.MAX_PAYLOAD_LENGTH || payloadLength < 0) {
            throw new LifeException.NotValidException("attempted to create a block with payloadLength " + payloadLength);
        }
        ...
        if (transactions != null) {
            ...
            if (blockTransactions.size() > Constants.MAX_NUMBER_OF_TRANSACTIONS) {
                throw new LifeException.NotValidException("attempted to create a block with " + blockTransactions.size() + " transactions");
            }
            long previousId = 0;
            for (Transaction transaction : this.blockTransactions) {
                if (transaction.getId() <= previousId && previousId != 0) {
                    throw new LifeException.NotValidException("Block transactions are not sorted!");
                }
    ...
    static BlockImpl parseBlock(JSONObject blockData) throws LifeException.ValidationException {
        try {
    ...
    byte[] getBytes() {
        ...
        if (version < 3) {
            buffer.putInt((int)(totalAmountNQT / Constants.ONE_LIFE));
            buffer.putInt((int)(totalFeeNQT / Constants.ONE_LIFE));
        } else {
    ...
    void sign(String secretPhrase) {
        if (blockSignature != null) { //TODO comment first run
    ...
    boolean verifyGenerationSignature() throws BlockchainProcessor.BlockOutOfOrderException {
        ...
        try {
            ...
            Account account = Account.getAccount(getGeneratorId());
            long effectiveBalance = account == null ? 0 : account.getEffectiveBalanceLIFE();
