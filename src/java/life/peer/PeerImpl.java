...
import life.util.CountingInputStream;
import life.util.CountingOutputStream;
...
final class PeerImpl implements Peer {
    ...
    public int getWeight() {
        ...
        if (hallmarkBalance == -1 || hallmarkBalanceHeight < LifeCoin.getBlockchain().getHeight() - 60) {
            long accountId = hallmark.getAccountId();
            Account account = Account.getAccount(accountId);
            hallmarkBalance = account == null ? 0 : account.getBalanceNQT();
            hallmarkBalanceHeight = LifeCoin.getBlockchain().getHeight();
        }
        return (int)(adjustedWeight * (hallmarkBalance / Constants.ONE_LIFE) / Constants.MAX_BALANCE_LIFE);
    }
    ...
    public void blacklist(Exception cause) {
        if (cause instanceof LifeException.NotCurrentlyValidException || cause instanceof BlockchainProcessor.BlockOutOfOrderException
    ...
    void connect() {
        ...
        if (response != null) {
            ...
            lastUpdated = LifeCoin.getEpochTime();
        } else {
            setState(State.NON_CONNECTED);
        }
    }
    ...
    boolean analyzeHallmark(String address, final String hallmarkString) {
        ...
        try {
            ...
            for (PeerImpl peer : groupedPeers) {
                peer.adjustedWeight = Constants.MAX_BALANCE_LIFE * peer.getHallmarkWeight(mostRecentDate) / totalWeight;
