...
import static life.user.JSONResponses.NOTIFY_OF_ACCEPTED_TRANSACTION;
...
public final class SendMoney extends UserServlet.UserRequestHandler {
    ...
    @Override
    JSONStreamAware processRequest(HttpServletRequest req, User user) throws LifeException.ValidationException, IOException {
        if (user.getSecretPhrase() == null) {
            return null;
        }

        String recipientValue = req.getParameter("recipient");
        String amountValue = req.getParameter("amountLIFE");
        String feeValue = req.getParameter("feeLIFE");
        ...
        try {
            ...
            amountNQT = Convert.parseLIFE(amountValue.trim());
            feeNQT = Convert.parseLIFE(feeValue.trim());
            ...
            response.put("amountLIFE", amountValue);
            response.put("feeLIFE", feeValue);
        ...
        } else if (feeNQT < Constants.ONE_LIFE || feeNQT > Constants.MAX_BALANCE_NQT) {

            JSONObject response = new JSONObject();
            response.put("response", "notifyOfIncorrectTransaction");
            response.put("message", "\"Fee\" must be at least 1 LIFE!");
            response.put("recipient", recipientValue);
            response.put("amountLIFE", amountValue);
            response.put("feeLIFE", feeValue);
            response.put("deadline", deadlineValue);
        ...
        if (account == null || Convert.safeAdd(amountNQT, feeNQT) > account.getUnconfirmedBalanceNQT()) {
            ...
            final Transaction transaction = LifeCoin.getTransactionProcessor().newTransactionBuilder(user.getPublicKey(),
