...
public final class GetNextBlockGenerators extends APIServlet.APIRequestHandler {
    ...
    JSONStreamAware processRequest(HttpServletRequest req) {
        ...
        if (block == null) {
            curBlock = LifeCoin.getBlockchain().getLastBlock();
        } else {
            try {
                curBlock = LifeCoin.getBlockchain().getBlock(Convert.parseUnsignedLong(block));
                if (curBlock == null) {
        ...
        */
        ...
        Block curBlock = LifeCoin.getBlockchain().getLastBlock();
